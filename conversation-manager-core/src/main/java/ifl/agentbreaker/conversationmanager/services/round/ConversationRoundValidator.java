...
public class ConversationRoundValidator
{
    ...
    public void validatePhaseFour(SaveConversationRoundRequest request)
    {
        ...
        if (request.getStatus() != RoundStatus.ROUND_STATUS_COMPLETED)
        {
            ...
            require(request.getTurnsCount() == 0,
                "Failed or cancelled rounds currently support only termination before the first completed turn.");
        ...
        require(request.getTurnsCount() == 1, "A completed round currently requires exactly one turn.");
        ...
        require(turn.getStatus() == TurnStatus.TURN_STATUS_COMPLETED && turn.getErrorMessage().isEmpty(),
            "The turn must be completed without an error.");
        ...
        require(turn.getToolCallExecutionsCount() == 0, "Tools are not enabled yet.");
    ...
    private void validateLlmCall(LlmCall call, ConversationTurn turn)
    {
        ...
        require(request.getToolsCount() == 0, "Tools are not enabled yet.");
        for (LlmConversationMessage message : request.getMessagesList())
        {
            ...
            require(message.getContentPartsCount() == 0 && message.getToolCallsCount() == 0
                ...
                "Request messages must be plain text without tool metadata.");
        ...
        require(response.getMessage().getContentPartsCount() == 0 && response.getMessage().getToolCallsCount() == 0,
            "LLM responses cannot contain multimodal content or tool calls yet.");
